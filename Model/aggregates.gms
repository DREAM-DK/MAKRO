...
$IF %stage% == "variables":
  ...
  ;
  $GROUP+ G_fixed_forecast G_aggregates_fixed_forecast;

  $GROUP G_aggregates_ARIMA_forecast
  ;
  $GROUP+ G_ARIMA_forecast G_aggregates_ARIMA_forecast;

  $GROUP G_aggregates_calibrated
    vBVT2hLsnit[t] "Glidende gennemsnit af BVT pr. arbejdstime i værdi."
    qBVT2hL[s_,t] "BVT pr. arbejdstime i mængde."
    qBVT2hLsnit[t] "Glidende gennemsnit af BVT pr. arbejdstime i mængde."
    vBVT2hL[s_,t] "BVT pr. arbejdstime i værdi."
    vBNI[t] "Bruttonationalindkomst, Kilde: ADAM[Yi]"
    vUdlNet[t] "Udlandets finansielle nettoportefølje ift. DK, Kilde: ADAM[Wn_e]."
    vUdlAkt[portf_,t] "Udlandets finansielle aktiver ift. DK, Kilde: jf. portfolio set."
    vUdlAkt[portf_,t] "Udlandets finansielle aktiver ift. DK, Kilde: jf. portfolio set."
    vUdlPas[portf_,t] "Udlandets finansielle passiver ift. DK, Kilde: jf. portfolio set."
    vUdlNFE[t] "Udlandets nettofordringserhvervelse, Kilde: ADAM[Tfn_e]"
    vBetalingsbalance[t] "Saldo på den officelle betalingsbalances løbende poster. Kilde: ADAM[Enl]."
    vUdlPensUdb[t] "Pensionsudbetalinger til udlandet, Kilde: ADAM[Typc_cf_e]"
    vUdlPensIndb[t] "Pensionsindbetalinger fra udlandet, Kilde: ADAM[Tpc_e_z]"
    vtPALudl[t] "PAL-skat betalt af udlændinge"
    vUdlPensAfk[t] "Afkast fra pensionsformue EFTER SKAT for udlændinge."
    vPens[pens_,t] "Samlet pensionsformue fordelt på pensionstype"
    vPensIndb[pens_,t] "Pensionsindbetalinger samlet til både husholdninger og udland fordelt på ordning"
    vPensUdb[pens_,t] "Pensionsudbetalinger samlet til både husholdninger og udland fordelt på ordning"
    vPensAfk[pens_,t] "Pensionsafkast samlet til både husholdninger og udland fordelt på ordning"
    vUdlNetRenter[t] "Udlandets nettoformueindkomst ift. DK, Kilde: ADAM[Tin_e]"
    vUdlAktRenter[portf_,t] "Samlet formueindkomst fra aktiver for finansielle og ikke-finansielle selskaber"
    vUdlPasRenter[portf_,t] "Samlet rente- og dividendeudskrivninger for finansielle og ikke-finansielle selskaber"
    vUdlOmv[t] "Omvurderinger af udenlandsk nettoformue, Kilde: ADAM[Wn_e]-ADAM[Wn_e][-1]-ADAM[Tfn_e]"
    vUdlAktOmv[portf_,t] "Omvurderinger på selskabernes finansielle aktiver"
    vUdlPasOmv[portf_,t] "Omvurderinger på selskabernes finansielle passiver"
    rUdlPensUdb[t] "Udenlændinges pensionsudbetalingsrate."
...
# ======================================================================================================================
$IF %stage% == "equations":
  $BLOCK B_aggregates_static G_aggregates_static $(tx0[t])
    .. vBVT2hL[s,t] =E= vBVT[s,t] / hL[s,t];
    
    .. vBVT2hL[sTot,t] =E= vBVT[sTot,t] / hL[sTot,t];

    .. vBVT2hLsnit[t] =E= vBVT2hLsnit[t-1] * 0.8 + 0.2 * vBVT2hL[sTot,t];

    .. qBVT2hL[s,t] =E= qBVT[s,t] / hL[s,t];

    .. qBVT2hL[sTot,t] =E= qBVT[sTot,t] / hL[sTot,t];
    ...
    $(t.val > %NettoFin_t1%).. 
      vUdlAkt[pensPortf,t] =E= vUdlAkt['pensTot',t-1]/fv + vUdlPensAfk[t] + vUdlPensIndb[t] - vUdlPensUdb[t];

    # Samlede pensionsindbetalinger, -udbetalinger og -afkast
    $(t.val >= %NettoFin_t1% and not PensX[pens])..
    ...
    $(t.val >= %NettoFin_t1% and (Obl[portf] or Bank[portf]))..
    ...
    vUdlAkt&_residual[portf,t]$(t.val >= %NettoFin_t1% and (IndlAktier[portf] or RealKred[portf]))..
    ...
    $(t.val >= %NettoFin_t1% and d1vUdlPas[portf,t] and not RealKred[portf])..
    ...
    $(t.val >= %NettoFin_t1%)..
      vUdlAkt[portfTot,t] =E= sum(portf, vUdlAkt[portf,t]);

    $(t.val >= %NettoFin_t1%)..
      vUdlPas[portfTot,t] =E= sum(portf, vUdlPas[portf,t]);
    ...
    $(t.val > %NettoFin_t1%)..
      vUdlAktRenter[pensPortf,t] =E= vPensionRenter[t] - vHhAktRenter[pensPortf,t];

    vUdlAktOmv&_pens[portf,t]$(t.val > %NettoFin_t1% and pensPortf[portf])..
    ...
    $(t.val > %NettoFin_t1% and d1vUdlAkt[portf,t] and not pensPortf[portf])..
    ...
    $(t.val > %NettoFin_t1% and d1vUdlPas[portf,t] and not d1vUdlAkt[portf,t])..
  ...
  $GROUP G_aggregates_endo G_aggregates_static, G_aggregates_forwardlooking_endo;
  model M_base / M_aggregates /;

  $GROUP+ G_Endo G_aggregates_endo;

  model M_static / B_aggregates_static /;
  $GROUP+ G_static G_aggregates_static;
$ENDIF
...
$IF %stage% == "static_calibration":
  ...
  /;
  model M_static_calibration / M_aggregates_static_calibration /;
  $GROUP+ G_static_calibration G_aggregates_static_calibration;

  $GROUP G_aggregates_static_calibration_newdata
    G_aggregates_static_calibration
  ;
  $GROUP+ G_static_calibration_newdata G_aggregates_static_calibration_newdata;
$ENDIF
...
$IF %stage% == "deep_dynamic_calibration":
  ...
  model M_deep_dynamic_calibration / M_aggregates_deep /;
  $GROUP+ G_deep_dynamic_calibration G_aggregates_deep;
$ENDIF
...
$IF %stage% == "dynamic_calibration_newdata":
  ...
  MODEL M_aggregates_dynamic_calibration / 
    M_aggregates
  /;
  model M_dynamic_calibration_newdata / M_aggregates_dynamic_calibration /;
  $GROUP+ G_dynamic_calibration_newdata G_aggregates_dynamic_calibration;
$ENDIF
