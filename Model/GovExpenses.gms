...
$IF %stage% == "variables":
  ...
  $GROUP G_GovExpenses_endo_a
    fHhOvf[t]$(t.val > %AgeData_t1%) "Korrektionsfaktor for overførslerindkomster, som sikrer afbalancering af aldersfordeling."
    fuOvfUbeskat[t]$(t.val > %AgeData_t1%) "Korrektionsfaktor for ubeskattede indkomstoverførsler, som sikrer afbalancering af aldersfordeling."
    fuPensIndbOP[t]$(t.val > %AgeData_t1%) "Korrektionsfaktor for obligatorisk opsparing, som sikrer afbalancering af aldersfordeling."
    ...
    vPensIndbOP[a_,t]$(a15t100[a_] and t.val >= 2020) "Obligatorisk pensionsopsparing af overførselsindkomster."
  ...
  $GROUP G_GovExpenses_endo
    ...
    vGKol[t]$(t.val >= %BFR_t1%) "Kollektivt offentligt forbrug. Kilde: ADAM[co]-ADAM[coi]"
    ...
    dvOvf2dnPop[ovf_,t]$(t.val >= %BFR_t1% and (ovf[ovf_] or HhTot[ovf_])) "Marginal ændring i udgifter til overførsel ovf ved ændring i den samlede befolkningsstørrelse."

    uvGInd[t]$(t.val >= %BFR_t1%) "Skala-parameter som bestemmer niveau for individuelt offentligt service før korrektion for demografisk træk og løn-niveau."
    ...
    dnOvf2dnPop[ovf,t]$(t.val >= %BFR_t1%) "Marginal ændring i antal modtagere af overførsel ovf ved ændring i den samlede befolkningsstørrelse."
...
$IF %stage% == "equations":
  $BLOCK B_GovExpenses_static$(tx0[t])
    ...
    E_uvGInd[t]$(t.val >= %BFR_t1%)..
      uvGxAfskr[t] =E= (uvGKol[t] * nPopInklOver100[t] + uvGInd[t] * fDemoTraek[aTot,t]) * (1+rGLukning[t]);

    E_vGInd[t].. vG[gTot,t] =E= vGKol[t] + vGInd[t];
    E_vGKol[t]$(t.val >= %BFR_t1%)..
    ...
    E_pGInput[t].. pGInput[t] =E= fpGInput[t] * pG[gTot,t] / pY['off',t] * pYOffInput[t];
    E_qGInput[t].. qGInput[t] * pGInput[t] =E= vG[gTot,t];
    E_qGxAfskrInput[t]..
      qGxAfskrInput[t] * pGxAfskrInput[t-1]/fp =E= pGInput[t-1]/fp * qGInput[t] 
                                                     - pOffAfskr[kTot,t-1]/fp * qOffAfskr[kTot,t];
    E_pGxAfskrInput[t].. pGxAfskrInput[t] * qGxAfskrInput[t] =E= vGxAfskr[t];
    ...
    E_vOvfSats_loenreg[loenreg,t]$(t.val >= %BFR_t1%)..
      vOvfSats[loenreg,t] =E= vLoenIndeks[t] * uvOvfSats[loenreg,t];

    E_vOvfSats_oblpens[oblpens,t]$(t.val >= %BFR_t1%)..
      vOvfSats[oblpens,t] =E= vSatsIndeksx[t] * uvOvfSats[oblpens,t];
    ...
    E_vOvfSats_intro[intro,t]$(t.val >= %BFR_t1%)..
      vOvfSats[intro,t] =E= vSatsIndeks[t] * uvOvfSats[intro,t];
    ...
    E_vOvf[ovf,t]$(t.val >= %BFR_t1% and not sameas['iskatpl',ovf])..
    ...
    E_vPensIndbOP_aTot[t]$(t.val >= 2020)..
      vPensIndbOP[aTot,t] =E= fvPensIndbOP[t] * rOblOpsp2Ovf[t] * sum(oblpens, vOvf[oblpens,t]);

    E_jvOvf_tot[t]$(t.val >= %BFR_t1%).. jvOvf['tot',t] =E= sum(ovf, jvOvf[ovf,t]);

    E_vOvf_tot[t]$(t.val >= %BFR_t1%).. vOvf['tot',t] =E= sum(ovf, vOvf[ovf,t]);
    E_vOvf_HhTot[t]$(t.val >= %BFR_t1%)..  vOvf['HhTot',t]  =E= sum(ovfHh, vOvf[ovfHh,t]);
    ...
    E_dnOvf2dnPop[ovf,t]$(t.val >= %BFR_t1%)..
    ...
    E_dvOvf2dnPop[ovf,t]$(t.val >= %BFR_t1%).. dvOvf2dnPop[ovf,t] =E= vOvfSats[ovf,t] * dnOvf2dnPop[ovf,t];
    E_dvOvf2dnPop_hh[t]$(t.val >= %BFR_t1%).. dvOvf2dnPop['HhTot',t] =E= sum(ovfHh, dvOvf2dnPop[ovfHh,t]);

    # Opdeling af ubeskattede og skattepligtige overførsler
    E_vOvfUbeskat_tot[t]$(t.val >= %BFR_t1%).. vOvfUbeskat[aTot,t] =E= sum(ubeskat, vOvf[ubeskat,t]);
    ...
    E_vSubYRest_Tot_via_rSubYRest[t].. vSubYRest[sTot,t] =E= rSubYRest[sTot,t] * vY[sTot,t];
    E_vSubYRest_Tot[t].. vSubYRest[sTot,t] =E= sum(s, vSubYRest[s,t]);
    ...
    E_uOffTilHhRest[t].. vOffTilHhRest[t] =E= uOffTilHhRest[t] * vSatsindeks[t] * nPopInklOver100[t];
  ...
  $BLOCK B_GovExpenses_a$(tx0[t])
    ...
    E_vOvf_HhTot_via_fHhOvf[t]$(t.val > %AgeData_t1%).. vOvf['HhTot',t] =E= sum(a, vHhOvf[a,t] * nPop[a,t]);
    ...
    E_vOvfUbeskat[a,t]$(a0t100[a] and t.val > %AgeData_t1%)..
      vOvfUbeskat[a,t] =E= fuOvfUbeskat[t] * uOvfUbeskat[a,t] * vOvfUbeskat[aTot,t];
    E_vOvfUbeskat_via_fuOvfUbeskat[t]$(t.val > %AgeData_t1%)..
      vOvfUbeskat[aTot,t] =E= sum(a, vOvfUbeskat[a,t] * nPop[a,t]);

    E_vOvfSkatPl[a,t]$(t.val > %AgeData_t1% and a0t100[a]).. 
      vOvfSkatPl[a,t] =E= vHhOvf[a,t] - vOvfUbeskat[a,t];

    # Aldersfordeling af obligatoriske pensionsindbetalinger holdes konstant ved stød - kan klart modelleres bedre
    E_vPensIndbOP_a[a,t]$(a15t100[a] and t.val >= 2020)..
      vPensIndbOP[a,t] =E= fuPensIndbOP[t] * uPensIndbOP[a,t] * vPensIndbOP[aTot,t];
    E_vPensIndbOP_via_fuPensIndbOP[t]$(t.val >= 2020)..
      vPensIndbOP[aTot,t] =E= sum(a, vPensIndbOP[a,t] * nPop[a,t]);
  $ENDBLOCK
...
$IF %stage% == "static_calibration":
  ...
  $GROUP G_GovExpenses_static_calibration
    ...
    -fHhOvf # -E_vOvf_HhTot_via_fHhOvf
    -fuPensIndbOP # - E_vPensIndbOP_via_fuPensIndbOP
    -fuOvfUbeskat # - E_vOvfUbeskat_via_fuOvfUbeskat
  ;
  $GROUP G_GovExpenses_static_calibration 
  ...
  MODEL M_GovExpenses_static_calibration /
    ...
    - E_vOvf_HhTot_via_fHhOvf
    - E_vPensIndbOP_via_fuPensIndbOP
    - E_vOvfUbeskat_via_fuOvfUbeskat
  /;
$ENDIF
